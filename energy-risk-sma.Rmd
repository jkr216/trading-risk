...
That's a random sample of 30 daily returns from our strategy, with replaceement. 

```{r}
sma_trend_results %>% 
  select(trend_returns) %>% 
   sample_n(30, replace = TRUE) %>% 
  ggplot(aes(x = trend_returns)) +
  geom_histogram(binwidth = .001, color = "cornflowerblue", fill = "pink")
```
...
bootstrapped %>% 
  ggplot(aes(x = mean)) +
  geom_histogram(binwidth = .0001, fill = "cornflowerblue") + 
  geom_segment(aes(x = mean(bootstrapped$upper_ci), xend = mean(bootstrapped$upper_ci), y = 0, yend = Inf)) +
  geom_segment(aes(x = mean(bootstrapped$lower_ci), xend = mean(bootstrapped$lower_ci), y = 0, yend = Inf))
...
Port this work to Shiny so end user can choose different assets, different moving averages, different z-score levels, and different backtest intervals - very important. 

Also include a monte carlo to simulate the future? 
